...
rows = []
lines = args.type_csv[0].readlines()
for l in lines:
...
'''.format(args.type_csv[0].name, sys.argv[0], args.namespace))
...
if len(args.type_csv) > 1:
    rows = []
    lines = args.type_csv[1].readlines()
    for l in lines:
        rows.append(tuple(l.strip().split(',')))
    for row in rows:
        printh(u"#define UA_NS2ID_{name} {id} /* {description} */".format(name=row[0].upper(), id=row[1], description=row[2]))
